...
export function tokenize(
  ...
  function skipWhitespaces() {
    while (
      offset < len &&
      isNonLineBreakWhitespace(source.charCodeAt(offset))
    ) {
      offset += 1
      column += 1
    }
  ...
  function next() {
    ...
    if (source.charCodeAt(offset) === 34 /* " */) {
      ...
      if (offset >= len) {
        onError(ErrorKind.UnterminatedQuotedString, {
          pos: offset,
          line,
          column,
        })
  ...
  return {
    expectLinebreak() {
      if (!this.eatLinebreak()) {
        ...
        onError(ErrorKind.ExpectLineBreak, {
    ...
    expectString(kind?: TokenKind.Quoted | TokenKind.Unquoted) {
      ...
      if (token) {
        ...
        onError(ErrorKind.ExpectTokenQuoted, { pos: offset, line, column })
        return {
          kind: TokenKind.Quoted,
          value: '',
          pos: offset,
        ...
        onError(ErrorKind.ExpectTokenUnquoted, { pos: offset, line, column })
        return {
          kind: TokenKind.Unquoted,
          value: '',
          pos: offset,
